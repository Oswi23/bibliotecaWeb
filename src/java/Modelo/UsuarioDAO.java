...
public class UsuarioDAO {
    ...
    public int obtenerUltimoNumeroPrestamo() {
        int ultimoNumero = 0;
        String sql = "SELECT MAX(id_prestamo) AS max_id FROM prestamo";
        try {
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            if (rs.next()) {
                ultimoNumero = rs.getInt("max_id") + 1;
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Map<String, Object>> buscarLibrosPorNombre(String nombreLibro) {
        List<Map<String, Object>> librosEncontrados = new ArrayList<>();
        String sql = "SELECT codigo_ejemplar, nombre_ejemplar FROM biblioteca WHERE nombre_ejemplar LIKE ?";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                libro.put("nombre", rs.getString("nombre"));
    ...
    public String obtenerNombreUsuario(String carnet) {
        String nombreUsuario = null;
        String sql = "SELECT nombre_user FROM usuario WHERE carnet = ?";
        ...
        try {
            ...
            if (rs.next()) {
                nombreUsuario = rs.getString("nombre_user");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
